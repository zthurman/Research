#!/usr/bin/env python
# Perturbation theory implementation of the Mandelbrot set cheese
...
def mandelbrot( h,w, maxit=300):
    ...
    bn = 0 + 0j		# b initial value
    cn = 0 + 0j		# c initial value
	...
	# alternate c initial values for zoom coordinates when that becomes applicable
        ...
        diverge = ((yn)*(conj(yn))).real > 30**30      # who is diverging, term on the right of the inequality determines level of detail
        div_now = diverge & (divtime==maxit)  # who is diverging now
        divtime[div_now] = i                  # note when
        z[diverge] = i                       # avoid diverging too much, when I first started z[diverge] = 2, now for shits and gigs set to i
    return divtime
